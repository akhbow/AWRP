...
class ImageProcessor:
    ...
    @staticmethod
    def save_awrp_for_trial(sub, trial, rp_generator, data_processor):
        for chn in range(Constants.NUM_CHANNEL):
            signal = data_processor.read_data(sub)[trial][chn]
            rps = []
            ...
            temp_filename = f'temp_image_sub_{sub}_trial_{trial}_ch_{chn}.png'
            ...
            try:
                if os.path.exists(temp_filename):
                    ...
                    filename = f'AWRP_DEAP_{Constants.SEGMENT_LENGTH}/sub_{sub+1:02d}_trial_{trial+1:02d}_ch_{chn+1:02d}.png'
...
def generate_and_record_awrp(args):
    sub, trial = args
    # Initialize classes
    subjects = [f's{index:02d}.mat' for index in range(1, 33)]
    ...
    # Generate AWRP
    image_processor.save_awrp_for_trial(sub, trial, rp_generator, data_processor)

    # Calculate memory used and running time
...
def main():
    ...
    try:
        os.makedirs(f'AWRP_DEAP_{Constants.SEGMENT_LENGTH}')
    except FileExistsError:
        pass

    with Pool() as pool:
        results = pool.map(generate_and_record_awrp, [(sub, trial) for sub in range(32) for trial in range(Constants.NUM_TRIAL)])
